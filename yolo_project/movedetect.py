...
# — YOLO’yu çalıştırıp labels klasörünü alıyoruz — 
    run_name = f"batch_predict_extractcells{uuid.uuid4()}{uuid.uuid4()}"
    ...
    subprocess.run([
        ...
        "model=../yolo_project2/runs/detect/train/weights/best.pt",
        f"source={image_path}",
        "save_txt=True",
        "project=runs/detect",
        f"name={run_name}",
    ...
    outputcells = f"foundcells{uuid.uuid4()}{uuid.uuid4()}"
    ...
    for idx, d in enumerate(ordered, start=1):
        ...
        if w < target_width:
        # Kenarlık boyutu hesapla
            total_padding = target_width - w
            left_pad = total_padding // 2
            right_pad = total_padding - left_pad

            # Beyaz kenarlık ekle (OpenCV'de beyaz: (255, 255, 255))
            ...
            cv2.imwrite(f"{outputcells}/crop_{idx:03d}.jpg", padded_crop) 
